...
export default function LearningPage() {
  ...
  const [creativePatterns, setCreativePatterns] = useState<CreativePattern[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setCreativePatterns(json.creativePatterns ?? []);
  }, []);
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        description="Admin Learning Lab — eval suites, agent version candidates, forecast calibration, and quality checks. Customer Analytics → Learning stays business-facing."
        actions={
          <button
      ...
      <section className="space-y-3">
        <h2 className="section-title">Creative patterns</h2>
        <p className="text-sm text-[var(--muted)]">
          Learned hooks, topics, CTAs, and formats — only when enough samples exist. Tiny samples stay marked as insufficient data.
        </p>
        {creativePatterns.length === 0 ? (
          <div className="surface p-4 text-sm text-[var(--muted)]">
            No creative patterns yet. Publish more content with performance data to unlock early patterns.
          </div>
        ) : (
          <ul className="grid gap-3 md:grid-cols-2">
            {creativePatterns.map((pattern) => (
              <li key={pattern.id} className="surface p-4 space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  <span
                    className={
                      pattern.showAsRecommendation
                        ? "badge badge-success"
                        : pattern.maturity === "INSUFFICIENT_DATA"
                          ? "badge"
                          : "badge badge-warn"
                    }
                  >
                    {pattern.maturityLabel}
                  </span>
                  <span className="badge">{pattern.sampleSize} samples</span>
                </div>
                <p className="font-medium text-sm">{pattern.label}</p>
                {!pattern.showAsRecommendation && (
                  <p className="text-xs text-[var(--muted)]">
                    Not shown as a recommendation until sample size clears the early-pattern threshold.
                  </p>
                )}
              </li>
            ))}
