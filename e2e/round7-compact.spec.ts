/**
 * Round 7 COMPACT — Workspace recovery 3/3, Inbox 5/5, Ask Go 3/3, Pipeline tile 3/3.
 * Clean profile + long-lived/migrated profile. Normal locator.click only.
 * No retries, coordinates, or force.
 */
...
const RUN_ID = `R7C-${Date.now()}`;
const STORAGE_CLEAN = path.join(process.cwd(), "QA", ".round7-storage-clean.json");
const STORAGE_LONG = path.join(process.cwd(), "QA", ".round7-storage-longlived.json");
...
test.describe("Round 7 compact", () => {
  ...
  test("Inbox 5/5 clean profile", async ({ browser }) => {
    ...
    for (const id of [a, b, c, a, b]) {
      console.log(`R7_INBOX_CLEAN=${pass + 1}/5 id=${id}`);
      await selectInbox(page, id);
      pass++;
    }
    console.log(`R7_INBOX_CLEAN_PASS=${pass}/5`);
    expect(pass).toBe(5);
    await context.close();
  });

  test("Ask Go 3/3 + Pipeline tile 3/3 clean", async ({ browser }) => {
    test.setTimeout(120_000);
    const clean = await ensureCleanState(browser);
    const { context, page } = await openWithState(browser, clean);
    let askPass = 0;
    ...
    for (let i = 0; i < 3; i++) {
      ...
      console.log(`R7_PIPE=${i + 1}/3`);
      await tile.click({ timeout: 10_000 });
      expect((await post).ok()).toBeTruthy();
      pipePass++;
    }
    console.log(`R7_ASK_PASS=${askPass}/3 R7_PIPE_PASS=${pipePass}/3`);
    expect(askPass).toBe(3);
    expect(pipePass).toBe(3);
    await context.close();
  });
  ...
  test("Long-lived profile Inbox+Ask+Pipeline", async ({ browser }) => {
    ...
    await waitReady(page);
    const goBtn = page.getByTestId("ask-go");
    await page.getByRole("textbox").first().fill(`E2E r7 longlived ${RUN_ID}`);
    await expect(goBtn).toBeEnabled({ timeout: 12_000 });
    ...
    console.log("R7_LONG_LIVED_PROFILE=PASS");
    await context.close();
  });
});
