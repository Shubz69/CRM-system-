...
describe("ledger Restrict — schema and forward migration", () => {
  ...
  it("greenfield init creates ledger FKs as ON DELETE RESTRICT", () => {
    const sql = readFileSync(
      join(process.cwd(), "prisma/migrations/20260812170000_init/migration.sql"),
      "utf8",
    );
    ...
    expect(sql).toMatch(/CREATE TABLE "FailedJob"/);
  });
});
